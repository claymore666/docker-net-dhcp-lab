...
# started once, and the operator account is in neither the libvirt nor the
# kvm group (issue #1/#3 direction) -- measured live, an idempotent re-run
# against a work dir whose VM had already started once failed genisoimage
# outright with "Permission denied" on the existing, now libvirt-qemu-owned
# file. Deleting it needs only write permission on $WORK, which the
# operator already has, regardless of who owns the file being deleted.
rm -f "$seed_iso"
