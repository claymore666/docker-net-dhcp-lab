#!/bin/bash
# Refusal tests for containment-preflight.sh (issue #1, task 2). CI-safe:
# every case stubs `nft` on PATH, never touches the real lab host or any
# real nftables state. Run by verify.sh on every push.
set -euo pipefail
...
# Case 4a: the hook at priority -10, numeric spelling. Must pass.
...
if ! PATH="$tmp/case-ok-numeric:$PATH" "$PREFLIGHT" >/dev/null 2>&1; then
	echo "containment-preflight-test: FAIL -- refused a valid numeric -10 hook" >&2
	fail=1
fi

# Case 4b: the hook at priority -10, symbolic spelling. Must pass.
...
if ! PATH="$tmp/case-ok-symbolic:$PATH" "$PREFLIGHT" >/dev/null 2>&1; then
	echo "containment-preflight-test: FAIL -- refused a valid 'filter - 10' hook" >&2
	fail=1
fi
...
echo "containment-preflight-test: PASS -- all 5 cases behaved as expected"
